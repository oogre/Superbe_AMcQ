...
def killAV():
	...
	if DEBUG is False :
		try:
			subprocess.Popen([DBUSCONTROL, "stop"], stdin=subprocess.PIPE, stdout=subprocess.PIPE);
...
def fade(alpha):
	...
	if DEBUG is False:
		try:
			subprocess.Popen([DBUSCONTROL, "setalpha", str(alpha)], stdin=subprocess.PIPE, stdout=subprocess.PIPE);
			subprocess.Popen([DBUSCONTROL, "volume", str(max(0.01, alpha / 255.0))], stdin=subprocess.PIPE, stdout=subprocess.PIPE);
